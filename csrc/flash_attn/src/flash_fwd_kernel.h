...
template<typename Kernel_traits, bool Is_causal, bool Is_local, bool Has_alibi, bool Is_even_MN, bool Is_even_K, bool Is_softcap, bool Split, bool Append_KV, bool Is_kv_bf16, typename Params>
inline __device__ void compute_attn_1rowblock_splitkv(const Params &params, const int bidb, const int bidh, const int m_block, const int n_split_idx, const int num_n_splits) {
    ...
    if (block_table != nullptr) {
        ...
        tKgK.data() = gK.data() + FLASH_NAMESPACE::resolve_thread_kv_page_slice_offset<Kernel_traits>(tidx, n_block_max - 1, params.page_block_size,
        ...
        tVgV.data() = gV.data() + FLASH_NAMESPACE::resolve_thread_kv_page_slice_offset<Kernel_traits>(tidx, n_block_max - 1, params.page_block_size,
        ...
        for (int n_block = n_block_max - 1; n_block >= n_block_copy_min; n_block--) {
            ...
            if (block_table == nullptr) {
                ...
                if (n_block > n_block_copy_min) {
                    tVgV.data() = gV.data() + FLASH_NAMESPACE::resolve_thread_kv_page_slice_offset<Kernel_traits>(tidx, n_block - 1, params.page_block_size, 
                        block_table, params.v_batch_stride, params.v_row_stride,
                        page_compress_cache, params.page_compress_topk, num_compressed_page);
                    tKgK.data() = gK.data() + FLASH_NAMESPACE::resolve_thread_kv_page_slice_offset<Kernel_traits>(tidx, n_block - 1, params.page_block_size, 
                        block_table, params.k_batch_stride, params.k_row_stride,
                        page_compress_cache, params.page_compress_topk, num_compressed_page);
                }
            }
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        if (masking_step > 0) {
            if (block_table == nullptr) {
                ...
                tVgV.data() = gV.data() + FLASH_NAMESPACE::resolve_thread_kv_page_slice_offset<Kernel_traits>(tidx, n_block, params.page_block_size,
                    block_table, params.v_batch_stride, params.v_row_stride,
                    page_compress_cache, params.page_compress_topk, num_compressed_page);
            }
            // 异步拷贝 V 到 smem
...
template<typename Kernel_traits, bool Is_causal, bool Is_local, bool Has_alibi, bool Is_even_MN, bool Is_even_K, bool Is_softcap, bool Split, bool Append_KV, bool Is_kv_bf16, typename Params>
inline __device__ void compute_attn_1rowblock_splitkv_kvquant(const Params &params, const int bidb, const int bidh, const int m_block, const int n_split_idx, const int num_n_splits) {

    using Element = typename Kernel_traits::Element;
    using ElementKV = uint8_t;
    using ElementAccum = typename Kernel_traits::ElementAccum;
    using index_t = typename Kernel_traits::index_t;
    ...
    // if (threadIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0) { printf("Is_even_MN = %d, is_cumulativ = %d, seqlen_k_cache = %d, actual_seqlen_k = %d\n", Is_even_MN, params.is_seqlens_k_cumulative, binfo.seqlen_k_cache, binfo.actual_seqlen_k); }
    // if (threadIdx.x == 0 && blockIdx.y == 1 && blockIdx.z == 0) { printf("params.knew_ptr = %p, seqlen_k_cache + seqlen_knew = %d\n", params.knew_ptr, binfo.seqlen_k_cache + (params.knew_ptr == nullptr ? 0 : params.seqlen_knew)); }
    // if (threadIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0) { printf("Is_even_MN = %d, Is_even_K = %d, Is_softcap = %d, Split = %d, Append_KV = %d\n", Is_even_MN, Is_even_K, Is_softcap, Split, Append_KV); }
    if (m_block * kBlockM >= binfo.actual_seqlen_q) return;
    ...
    if (n_block_min >= n_block_max) {  // This also covers the case where n_block_max <= 0
        ...
        // Clear_OOB_K must be false since we don't want to write zeros to gmem
        FLASH_NAMESPACE::copy<Is_even_MN, Is_even_K, /*Clear_OOB_MN=*/false, /*Clear_OOB_K=*/false, /*Is_kv_bf16=*/false>(
            gmem_tiled_copy_Oaccum, tOrOaccum, tOgOaccum, tOcO, tOpO, binfo.actual_seqlen_q - m_block * kBlockM
        );
        #pragma unroll
        for (int m = 0; m < size<1>(tOgOaccum); ++m) {
    ...
    float k_scale = 1.0f;
    float v_scale = 1.0f;
    if (params.k_descale_ptr != nullptr){
        k_scale = params.k_descale_ptr[0];
        // printf("k_scale = %f\n", k_scale);
    }
    if (params.v_descale_ptr != nullptr){
        v_scale = params.v_descale_ptr[0];
        // printf("v_scale = %f\n", v_scale);
    }
    ...
    // Read Q from gmem to smem, optionally apply rotary embedding.
    if (!Append_KV || params.rotary_dim == 0) { //  params.rotary_dim = 0
        // We don't need to clear the sQ smem tiles since we'll only write out the valid outputs
        FLASH_NAMESPACE::copy<Is_even_MN, Is_even_K>(gmem_tiled_copy_Q, tQgQ, tQsQ, tQcQ, tQpQ,
                                                     binfo.actual_seqlen_q - m_block * kBlockM);
    } else {
    ...
    int n_block = n_block_max - 1;
    // We don't need to clear the sK smem tiles since we'll mask out the scores anyway.
    FLASH_NAMESPACE::copy<Is_even_MN, Is_even_K, /*Clear_OOB_MN=*/false, /*Clear_OOB_K=*/true, /*Is_kv_bf16=*/false>(gmem_tiled_copy_KV, tKgK, tKsK, tKVcKV, tKVpKV,
                                                 binfo.actual_seqlen_k - n_block * kBlockN, k_scale);
    cute::cp_async_fence();
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        if (masking_step > 0) {
            ...
            FLASH_NAMESPACE::copy</*Is_even_MN=*/true, Is_even_K, /*Clear_OOB_MN=*/false, /*Clear_OOB_K=*/true, /*Is_kv_bf16=*/false>(gmem_tiled_copy_KV, tVgV, tVsV, tKVcKV, tKVpKV, 0, v_scale);
        } else {
            // Clear the smem tiles to account for predicated off loads
            FLASH_NAMESPACE::copy<Is_even_MN, Is_even_K, /*Clear_OOB_MN=*/true, /*Clear_OOB_K=*/true, /*Is_kv_bf16=*/false>(
                gmem_tiled_copy_KV, tVgV, tVsV, tKVcKV, tKVpKV, binfo.actual_seqlen_k - n_block * kBlockN, v_scale
            );
        }
        cute::cp_async_fence();
        ...
        if (n_block > n_block_min) {
            ...
            FLASH_NAMESPACE::copy</*Is_even_MN=*/true, Is_even_K, /*Clear_OOB_MN=*/false, /*Clear_OOB_K=*/true, /*Is_kv_bf16=*/false>(gmem_tiled_copy_KV, tKgK, tKsK, tKVcKV, tKVpKV, 0, k_scale);
            // This cp_async_fence needs to be in the if block, otherwise the synchronization
            // isn't right and we get race conditions.
            cute::cp_async_fence();
        }
...
inline __device__ void compute_attn_splitkv(const Params &params) {
    ...
    if (Is_kv_bf16)
    {
        FLASH_NAMESPACE::compute_attn_1rowblock_splitkv<Kernel_traits, Is_causal, Is_local, Has_alibi, Is_even_MN, Is_even_K, Is_softcap, Split, Append_KV, Is_kv_bf16>(params, bidb, bidh, m_block, n_split_idx, num_n_splits);
    } else {
        FLASH_NAMESPACE::compute_attn_1rowblock_splitkv_kvquant<Kernel_traits, Is_causal, Is_local, Has_alibi, Is_even_MN, Is_even_K, Is_softcap, Split, Append_KV, Is_kv_bf16>(params, bidb, bidh, m_block, n_split_idx, num_n_splits);
    }
